...
class FSShell():
    def __init__(self, RawBlocks, FileOperationsObject, AbsolutePathNameObject):
        ...
        self.AbsolutePath = AbsolutePathNameObject
        ...
        while block_index <= (inobj.inode.size // fsconfig.BLOCK_SIZE):
            ...
            while current_position < end_position:
                ...
                if inobj2.inode.type == fsconfig.INODE_TYPE_DIR:
                    ...
                    inobj2.inode.type = fsconfig.INODE_TYPE_FILE
                    inobj2.StoreInode(self.RawBlocks)
                    data, errorcode = self.FileOperationsObject.Read(entryinodenumber, 0, fsconfig.MAX_FILE_SIZE)
                    if data == -1:
                        print("Error: " + errorcode)
                        continue
                    
                    print("[" + str(inobj2.inode.refcnt) + "]:" + entryname.decode() + "@ -> "+data.decode())
                    inobj2.inode.type = fsconfig.INODE_TYPE_SYM
                    inobj2.StoreInode(self.RawBlocks)
                else:
                    print("[" + str(inobj2.inode.refcnt) + "]:" + entryname.decode())
    ...
    def cat(self, filename):
        ...
        if inobj.inode.type == fsconfig.INODE_TYPE_SYM:
            ...
            inobj.StoreInode(self.RawBlocks)
            data, errorcode = self.FileOperationsObject.Read(i, 0, fsconfig.MAX_FILE_SIZE)
            if(data == -1):
                print("Error: "+ errorcode)
                return -1
            inode_number,type = self.AbsolutePath.PathNameToInodeNumber(self.cwd, data.decode())
            if type == 0:
                data2, errorcode2 = self.FileOperationsObject.Read(inode_number, 0, fsconfig.MAX_FILE_SIZE)
                if data2 == -1:
                    print("Error: " + errorcode2)
                    return -1
                print(data2.decode())
                inobj.inode.type = fsconfig.INODE_TYPE_SYM
                inobj.StoreInode(self.RawBlocks)
                return 0
            else:
                return -1
    ...
    def append(self, filename, string):
        ...
        if inobj.inode.type == fsconfig.INODE_TYPE_SYM:
            ...
            inode_number,type = self.AbsolutePath.PathNameToInodeNumber(self.cwd, data.decode())
            if type == 0:
                instance = InodeNumber(inode_number)
                instance.InodeNumberToInode(self.RawBlocks)
                written, errorcode = self.FileOperationsObject.Write(inode_number, instance.inode.size, bytearray(string, "utf-8"))
                if written == -1:
                    print("Error: " + errorcode)
                    return -1
                inobj.inode.type = fsconfig.INODE_TYPE_SYM
    ...
    def mirror(self, filename):
        ...
        if inobj.inode.type == fsconfig.INODE_TYPE_SYM:
            ...
            inode_number,type = self.AbsolutePath.PathNameToInodeNumber(self.cwd, data.decode())
            if type == 0:
                data2, errorcode2 = self.FileOperationsObject.Mirror(inode_number)
                if data2 == -1:
                    print("Error: " + errorcode2)
                    return -1
                inobj.inode.type = fsconfig.INODE_TYPE_SYM
    ...
    def lnh(self, target, name):
        i, errorcode = self.AbsolutePath.Link(self.cwd, target, name)
        if i == -1:
            print("Error: " + errorcode + "\n")
            return -1
        return 0
    ...
    def lns(self, target, name):
        i, errorcode = self.AbsolutePath.SymLink(self.cwd, target, name)
        if i == -1:
            print("Error: " + errorcode + "\n")
            return -1
        return 0
